...
class JsonDeliverStore(JsonStoreMaster):
    """Main subclass for delivered packages"""
    class __JsonDeliverStore(JsonStoreMaster):
        """Private JsonDeliverStore"""
        def __init__(self):
    ...
    def __new__(cls):
        if not JsonDeliverStore.instance:
            JsonDeliverStore.instance = JsonDeliverStore.__JsonDeliverStore()
        return JsonDeliverStore.instance
